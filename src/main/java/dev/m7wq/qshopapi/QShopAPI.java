...
import dev.m7wq.qshopapi.base.PaymentAdapter;
...
public class QShopAPI {
     ...
     */
    public void handlePurchase(Messager messages, List<String> defaultLore, InventoryClickEvent event, Material balance) {
     ...
     * @param event       the InventoryClickEvent triggered by the player's click in the shop.
     */
    public void handlePurchase(Messager messages, PaymentAdapter adapter, List<String> defaultLore, InventoryClickEvent event) {
    ...
    public Inventory handleShopGui(ShopAdapter shop) throws NullPointerException, IllegalStateException {
        ...
        int size = shop.getClass().getAnnotation(Size.class).size();
        String title = colorize(shop.getClass().getAnnotation(Title.class).title());

        String[] lore = shop.getClass().getAnnotation(Lore.class).lore();
